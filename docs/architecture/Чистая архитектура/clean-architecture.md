...
> Архитектурные рекомендации зависят от размера и требований проекта; примеры не являются обязательным шаблоном каталогов.

**Перед чтением:** пакеты, интерфейсы, ошибки и транзакции. **После главы:** вы
сможете провести границы предметной области и прикладного сценария, направить
зависимости и выбрать транзакционную границу. [Задания и самопроверка](../../practice.md#clean-architecture).
...
**Порт** — контракт на границе приложения, обычно интерфейс. **Адаптер** — код,
который преобразует конкретный внешний протокол или хранилище к этому контракту.
...
**Входящий порт** описывает операции, которые приложение предоставляет внешнему
миру. Его вызывает входящий адаптер: обработчик HTTP или gRPC, консольная команда
либо подписчик очереди.

**Исходящий порт** описывает то, что приложению нужно извне: сохранить заказ,
отправить уведомление, получить курс валют. Его реализует исходящий адаптер:
репозиторий SQL, клиент HTTP, файловое хранилище или издатель сообщений.

**Логика приложения** (сценарии использования) координирует предметные правила и
порты. Она не обязана быть «тонкой», но не должна зависеть от конкретной базы
данных, адреса внешнего сервиса или веб-библиотеки.
...
Самый внутренний слой — **предметная область**. Здесь живут типы и правила,
представляющие ключевые понятия системы: пользователь, заказ, товар, счёт,
бронирование. Например, правило о невозможности подтвердить заказ без адреса
доставки относится именно сюда.

Предметный код не должен знать о способе доставки запросов и хранения данных.
Это не означает запрет на любые сторонние пакеты: допустима библиотека, которая
действительно выражает предметную модель и не связывает её с инфраструктурой.
Зависимость оценивают по назначению, а не только по месту её публикации.
...
В Go этот принцип удобно реализовать благодаря неявному удовлетворению
интерфейсов: структура внешнего пакета реализует интерфейс внутреннего пакета без
специального объявления. Однако пакет реализации обычно всё равно импортирует
доменные типы из сигнатур; не требуется лишь явная связь вида `implements`.

### Пересечение границ: DTO

На внешней границе часто полезны отдельные структуры передачи данных: например,
JSON-запрос не должен становиться предметной сущностью только из-за совпадения
полей. Но создавать новый тип на каждой внутренней границе не обязательно.
Предметную сущность нормально передавать из сценария в репозиторий: зависимость
внешнего адаптера от внутреннего типа направлена правильно. Нельзя делать
обратное — передавать тип веб-библиотеки или ORM во внутренний слой.

Дополнительный тип оправдан, если формы данных или причины их изменения
различаются. Возможное размещение выглядит так:
...
Преобразования на внешних границах делают явными. Внутри приложения не стоит
копировать одинаковые структуры только ради формального соблюдения слоёв.
...
> **Зачем это Go-разработчику.** Каталог `internal` ограничивает импорт по дереву
> каталогов: пакет внутри `a/internal/x` доступен только коду внутри дерева `a`.
> Это полезная граница видимости, но она не проверяет направление зависимостей
> между вашими внутренними слоями.
...
| `internal/` | Код с ограниченным импортом: доступен только пакетам внутри дерева, содержащего каталог `internal`. Граница не обязана совпадать с модулем. |
...
> **Зачем это Go-разработчику.** Код преобразования между доменом и ORM — это Шаблонный код, но он механический и пишется один раз. В обмен вы получаете сценарии использования, которые можно тестировать без базы данных, и свободу менять ORM без переписывания бизнес-логики.

### Альтернативы ORM: от чистого SQL до sqlx

ORM — не единственный способ работы с базой данных в Go. В сообществе распространены три альтернативных подхода, и каждый из них по-разному вписывается в чистую архитектуру. Выбор между ними — это выбор компромисса между контролем над SQL и количеством шаблонного кода.
...
* `Scan()` вручную — много шаблонного кода, особенно при JOIN-запросах с десятком колонок.
...
* Сложные запросы со специвозможностейным для конкретной БД синтаксисом (например, `ON CONFLICT` в PostgreSQL) могут требовать «сырых» вставок через `sq.Expr()`.
...
| Подход          | Генерация SQL    | Маппинг строк        | Зависимости               | Контроль над SQL | Шаблонный код |
| --------------- | ---------------- | -------------------- | ------------------------- | ---------------- | ----------- |
...
* **sqlx** — лучший компромисс для большинства проектов. Сохраняет полный контроль над SQL, давая `StructScan` для избавления от Шаблонный код. Прекрасно уживается с чистой архитектурой: SQL-модели (`orderRow`) с тегами `db` живут строго в адаптере и никогда не просачиваются в домен.
...
> **Зачем это Go-разработчику.** Связка `sqlx` + доменные структуры — практичный компромисс. Вы получаете контроль над SQL, сокращаете ручной `Scan()` и не переносите структуры хранения в предметную область.

## 7. Сквозные решения

### Транзакция как граница сценария

Транзакцией управляет сценарий использования, потому что только он знает, какие
изменения должны быть атомарны. Репозиторий не должен незаметно начинать отдельную
транзакцию на каждый метод. Один из вариантов — передать функции набор репозиториев,
привязанных к `*sql.Tx`:
...
Адаптер базы данных отвечает за `BeginTx`, откат при ошибке и фиксацию. Сценарий
задаёт последовательность предметных операций. Не включайте сетевой вызов внутрь
долгой транзакции без осознанной причины: блокировки базы переживут срок сети.

### Ошибки, контекст и безопасность

- контекст входит первым аргументом в операции ввода-вывода и проходит через слои;
- адаптер переводит ошибки драйвера в устойчивый договор репозитория;
- транспорт переводит предметные ошибки в HTTP- или gRPC-коды и не раскрывает SQL;
- аутентификация устанавливает подтверждённую личность на внешней границе;
- авторизация остаётся частью сценария или предметной политики, где известны ресурс и действие.

Миграции принадлежат адаптеру хранения и развёртыванию, но совместимость схемы —
архитектурное требование. Сначала выпускайте код, понимающий старую и новую схему,
затем переносите данные и только после этого удаляйте старое поле.

### Размер проекта и разумная структура

| Размер | Достаточная отправная точка |
|---|---|
| маленький | `main`, пакет предметной логики, один пакет транспорта и хранения |
| средний | `cmd/`, `internal/domain`, `internal/app`, `internal/adapters` |
| большой | несколько предметных модулей с собственными границами; общими остаются только действительно стабильные средства |

Не копируйте полное дерево каталогов в учебный или небольшой сервис. Слои нужны,
когда они защищают направление зависимостей и упрощают изменение, а не ради числа папок.
...
**Основной источник:** [описание чистой архитектуры Роберта Мартина](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html).

## Краткий итог

- Зависимости направляют к устойчивым правилам предметной области.
- Интерфейс оправдан на границе, где потребителю действительно нужна смена реализации.
- Транзакционная граница охватывает целый прикладной сценарий, а не отдельный запрос.

Дальше: примените границы слоёв в [сквозной HTTP-службе](../../practice.md#capstone).
