...
**Перед чтением:** HTTP/2, контекст, интерфейсы и основы распределённых систем.
**После главы:** вы сможете описать совместимый контракт Protobuf, собрать вызов с
дедлайном и перевести предметные ошибки в коды gRPC. [Задания и самопроверка](../../practice.md#grpc).
...
> **Зачем это Go-разработчику.** Понимание того, как именно Go включает HTTP/2, позволяет осознанно настраивать сервер. В рабочей среде большинство gRPC-сервисов работает через HTTPS — HTTP/2 включается автоматически, без дополнительных действий.
...
| **Двунаправленный** | `rpc Method(stream Request) returns (stream Response)` | Чат, реального времени коллаборация, длительные задачи |
...
> **Зачем это Go-разработчику.** Серверный стриминг — основной паттерн для потоковой выдачи данных (логи, метрики, экспорт). Клиентский стриминг незаменим при пакетной загрузке. Двунаправленный стриминг — основа реального времени возможностей. Правило «Trailer() только после EOF» — частая причина ошибок при отладке.
...
> **Зачем это Go-разработчику.** Правильная обработка ошибок и deadlines — то, что отличает готового к эксплуатации gRPC-сервис от прототипа. Маппинг ошибок Go → gRPC позволяет использовать стандартные ошибки и не думать о кодах, пока не потребуется специвозможностейная логика. Пакет `status` с методом `WithDetails` даёт возможность пробрасывать структурированную информацию об ошибке клиенту.
...
* **Повтор** — повтор некоторых неудачных запросов на основе кодов ответа.
* **Конечный срок** — ограничение времени вызова.
* **Журналирование** — поддержка zap, logrus и slog.

**Мониторинг:** интеграция с Prometheus (метрики клиента и сервера) и OpenTelemetry (распределённая трассировка).

> **Зачем это Go-разработчику.** Перехватчики централизуют аутентификацию,
> наблюдаемость и восстановление после паники. Закрепляйте версию
> `go-grpc-middleware` и сверяйте примеры с её документацией: состав модулей и API
> меняются. Повторы допустимы только для идемпотентной операции и должны иметь
> конечный общий бюджет, случайную добавку к задержке и предел попыток.

***

## 10. Подготовка gRPC-сервиса к эксплуатации

Минимальный учебный сервер с `grpc.NewServer()` не задаёт эксплуатационную
политику. Перед развёртыванием определите:

* TLS, а при взаимной аутентификации — проверку клиентских сертификатов;
* аутентификацию и авторизацию каждого метода;
* максимальный размер входящего и исходящего сообщения;
* конечные сроки, параметры поддержания соединения и ограничения числа потоков;
* стандартную проверку здоровья и, только в доверенной сети, отражение схемы;
* метрики, распределённую трассировку и ограничение разнообразия меток;
* штатное завершение через `GracefulStop` с конечным сроком и запасным `Stop`.

Поток HTTP/2 обеспечивает управление потоком байтов, но не ограничивает вашу
бизнес-очередь автоматически. Если получатель обрабатывает сообщения медленнее
источника, задайте предел буфера и политику обратного давления. Клиентский
`CloseSend` закрывает только направление отправки; контекст по-прежнему нужен
для отмены всего вызова.

Для проверки сервера без сетевого порта используйте `bufconn`. В испытаниях
эволюции схемы сохраняйте неизвестные поля, никогда не переиспользуйте номера
удалённых полей и отмечайте их `reserved`. Версии `protoc`, `protoc-gen-go` и
`protoc-gen-go-grpc` закрепляйте в средствах сборки либо используйте настроенный
`buf`.

Пример воспроизводимого набора команд с Buf:
...
Версии модулей генерации фиксируйте в `buf.gen.yaml`, а версию самого `buf` — в файле
средств разработки или образе сборки. Сгенерированные файлы либо храните в репозитории
и проверяйте отсутствие расхождений, либо одинаково генерируйте перед каждой сборкой.
Удалённые модули генерации указаны с версиями, поэтому повторный `buf generate`
не выбирает новый выпуск неявно.

Клиент gRPC получает адреса через преобразователь имён. Политика `pick_first`
выбирает одно соединение, `round_robin` распределяет вызовы между готовыми адресами.
Выбор должен соответствовать служебному обнаружению и балансировщику; изменение строки
цели или конфигурации службы может изменить фактическую политику.

Для OpenTelemetry устанавливайте клиентские и серверные обработчики статистики,
передавайте контекст трассировки через метаданные и записывайте имя полного RPC-метода,
код результата и длительность. Идентификаторы пользователей и содержимое сообщений
не подходят для меток метрик. Проверяйте, что повторные попытки видны как попытки
одной логической операции, а не скрывают рост ошибок.
...
> **Зачем это Go-разработчику.** `net/rpc` — компактный способ понять идею RPC без внешних зависимостей. Понимание сигнатуры RPC-метода (два аргумента, error) полезно, так как gRPC наследует эту модель. В рабочих проектах `net/rpc` практически не используется — для современных приложений gRPC является стандартным выбором.
...
**Основные источники:** [документация gRPC для Go](https://grpc.io/docs/languages/go/) и [руководство Protocol Buffers](https://protobuf.dev/programming-guides/proto3/).

## Краткий итог

- Схема Protocol Buffers является версионируемым договором между клиентом и сервером.
- Срок вызова, код состояния и порядок перехватчиков должны быть частью проектирования API.
- Потоковый вызов требует явного ограничения сообщений и корректного завершения обеих сторон.

Дальше: отработайте эти договоры в [заданиях по gRPC](../../practice.md#grpc).
