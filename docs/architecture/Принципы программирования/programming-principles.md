...
> Принципы рассматриваются как условия и компромиссы, а не как безусловные правила.

**Перед чтением:** функции, структуры, интерфейсы и пакеты. **После главы:** вы
сможете обнаружить лишнюю связность, оценить конфликт принципов и обосновать
простое устройство пакета. [Задания и самопроверка](../../practice.md#principles).
...
> **Зачем это Go-разработчику.** Большой интерфейс стоит проверить: действительно
> ли каждый потребитель использует весь контракт? В Go небольшие интерфейсы часто
> определяют на стороне потребителя. Это эвристика, а не лимит в три метода:
> цельный контракт может быть больше, если операции не имеют смысла по отдельности.
...
> **Зачем это Go-разработчику.** Если `OrderService` сам открывает PostgreSQL,
> предметную логику трудно проверить отдельно от базы. Передавайте зависимость
> снаружи. Интерфейс нужен, когда потребителю действительно достаточно части
> поведения; для простых зависимостей допустим и конкретный тип или функция.
...
Оптимизировать следует подтверждённое узкое место, сохраняя корректность и
понятность кода. Это широко известная рекомендация, а не отдельный общепринятый
принцип с названием APO. При этом заранее выбрать подходящий алгоритм и пределы
ресурсов — не преждевременная оптимизация, а часть проектирования.
...
**Контекст Go.** Повторение устраняют функциями, именованной композицией и
обобщёнными типами. Встраивание продвигает поля и методы и тем самым меняет
открытый набор методов типа; оно не является безусловно лучшей заменой явному
полю и делегированию.
...
> **Зачем это Go-разработчику.** Заранее зафиксируйте требования, границы данных
> и дорогие для изменения внешние контракты. Небольшие внутренние интерфейсы часто
> лучше выделять по фактическим потребностям потребителя, а не проектировать все
> до появления рабочего кода.
...
> **Зачем это Go-разработчику.** Транзакция нужна для сохранения конкретного
> инварианта, а не просто потому, что запрос затронул несколько строк. Один запрос
> тоже может требовать транзакционного контекста, а несколько независимых
> изменений — нет. `defer tx.Rollback()` безопасно страхует ранний выход;
> изоляцию и блокировки выбирают под сценарий конкурирующих операций. Подробности
> вынесены в главу о базах данных.
...
> **Зачем это Go-разработчику.** Go поощряет композицию, но оставляет выбор между
> именованным полем, встраиванием и явным делегированием. Встраивайте тип, только
> если продвинутые методы действительно должны стать частью API внешнего типа;
> для простого переиспользования часто яснее именованное поле.
...
## 7. Как выбирать между принципами

Принцип полезен только вместе с условиями применения и контрпримером:

| Принцип | Когда помогает | Когда вредит |
|---|---|---|
| SRP | изменения приходят по разным причинам | класс дробят на обёртки без самостоятельной ответственности |
| OCP | есть стабильный контракт и несколько расширений | интерфейс создают заранее для единственной реализации |
| DRY | повторяется одно правило предметной области | внешне похожие фрагменты меняются независимо |
| KISS | простое решение выполняет измеримые требования | «простотой» оправдывают отсутствие обработки отказов |
| YAGNI | будущий сценарий не подтверждён | уже известное требование откладывают до дорогой переделки |
| DIP | нужно изолировать политику от внешней детали | интерфейс механически ставят между каждым вызовом |

Например, DRY предлагает объединить две проверки скидки, а SRP может потребовать
оставить их раздельными, если правила продаж и поддержки меняются разными владельцами.
Решение принимают по причинам изменения, а не по сходству строк.

Связность модуля оценивайте вопросом: «его части вместе реализуют одно правило или
собраны по техническому признаку?». Зависимости между пакетами можно увидеть командами:
...
Первая команда показывает транзитивные зависимости, вторая — прямые связи пакетов,
третья — граф модулей. Цикл импортов остановит компилятор, но отсутствие цикла ещё не
означает хорошую архитектуру: следите также за направлением связей и размером открытого
API пакетов.
...
| **LSP**        | Реализация соблюдает контракт  | Проверяйте не только сигнатуру методов          | Для полиморфного кода |
| **ISP**        | Потребитель не зависит от лишнего | Определяйте цельный минимальный контракт      | Когда интерфейс имеет разных потребителей |
| **DIP**        | Внутренняя политика не знает детали | Передавайте внешние зависимости снаружи      | На значимых архитектурных границах |
...
| **Композиция** | Собирайте поведение из частей  | Именованные поля, делегирование, иногда встраивание | Когда части имеют самостоятельную ответственность |
