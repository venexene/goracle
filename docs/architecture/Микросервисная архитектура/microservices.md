...
> Архитектурные рекомендации зависят от нагрузки, команды и требований к отказоустойчивости; универсальной схемы разбиения нет.

**Перед чтением:** HTTP или RPC, базы данных и границы прикладных сценариев.
**После главы:** вы сможете оценить цену выделения сервиса, спроектировать
идемпотентную обработку и совместимое изменение сообщения. [Задания и самопроверка](../../practice.md#microservices).
...
Продукт — это не набор реализованных возможностей, а непрерывные отношения с
...
## 6. Умные конечные точки и простые каналы (Smart Endpoints, Dumb Pipes) { #smart-endpoints-dumb-pipes }
...
Децентрализация данных влияет на способ их изменения. Внутри каждого сервиса
обычно сохраняются локальные ACID-транзакции. Для инварианта между сервисами
выбирают модель отдельно: синхронную проверку, резервирование, сагу, журнал
событий или, в редких случаях, распределённую транзакцию. Итоговая
согласованность — один из компромиссов, а не обязательная замена строгой
согласованности.
...
> Сага и итоговая согласованность полезны, когда бизнес допускает временное
> расхождение и определены компенсации. Если расхождение недопустимо, граница
> сервиса или выбранный протокол требуют пересмотра.
...
> это `context.Context` для общего бюджета времени, библиотеки размыкателя цепи
> и ограниченные повторы с растущей задержкой. Не создавайте механически новый
> тайм-аут вокруг каждого вызова: передавайте общий срок и выделяйте этапу лишь
> его часть, когда это требуется.
...
> `cockroachdb/circuitbreaker`. Часто комбинируется с ограниченными повторами.
...
Запись предметных данных и отправка сообщения брокеру не образуют одну обычную
транзакцию. Шаблон **transactional outbox** записывает изменение и событие в
одной локальной транзакции, после чего отдельный издатель доставляет событие.
На стороне получателя **inbox** или ключ идемпотентности позволяют безопасно
игнорировать повтор.

Формулировка «как минимум один раз» означает, что сообщение может прийти
повторно; «не более одного раза» допускает потерю. Заявление «ровно один раз»
имеет смысл только в чётко заданной границе и обычно строится на идемпотентной
обработке. Ограничивайте число попыток, добавляйте случайный разброс задержки и
помещайте неисправимые сообщения в отдельную очередь для разбора.
...
**Повтор** — простейший механизм повышения отказоустойчивости: многие
...
поэтому повтор обычно сочетается с **экспоненциально растущей задержкой** и
случайным разбросом:
задержка между попытками растет экспоненциально (100 мс → 200 мс →
400 мс → ...).

Повтор работает вместе с автоматическим выключателем: он помогает при временных
...
> общим контекстом с конечным сроком; в gRPC повторы включаются конфигурацией
> сервиса и не должны применяться к неидемпотентному методу без ключа операции.
...
> **Go:** приложение обычно пишет журналы в стандартные потоки вывода, а агент
> узла собирает их на уровне кластера. Дополнительный контейнер оправдан, если
> нужен отдельный жизненный цикл преобразования или доставки, но это не общее
> требование Kubernetes.
...
BFF не должен становиться владельцем основных предметных инвариантов. При этом
политика представления, агрегация, последовательность клиентских вызовов и
адаптация ошибок как раз относятся к его ответственности.
...
Фундаментальный принцип MSA, подробно рассмотренный в [секции 6](#smart-endpoints-dumb-pipes). Вся
...
Сложности: нелинейная модель взаимодействия, необходимость распределённой
трассировки, версия схемы события, идемпотентность обработчиков, ограничение
повторов и отдельная очередь для неисправимых сообщений.
...
## 17. Эволюция и эксплуатационный договор

Изменение схемы сообщения или API выполняйте по принципу «расширить — перевести —
сжать»: добавить совместимое поле, обновить потребителей, перенести данные, прекратить
запись старого поля и лишь затем удалить его. Для Protobuf не переиспользуйте номера
удалённых полей; для событий указывайте версию и проверяйте потребителей контрактными
тестами. Одновременное обновление всех сервисов не должно быть условием выпуска.

**SLO** задаёт измеримую цель для пользовательского результата, например долю успешных
запросов и задержку за окно времени. Бюджет ошибок показывает, сколько отказов допустимо
до остановки рискованных выпусков. Метрики отдельных контейнеров полезны для диагноза,
но не заменяют показатель всей операции пользователя.

Контекст трассировки нужно передавать через стандартные заголовки HTTP, метаданные gRPC
и свойства сообщения. Новый потребитель сообщения создаёт дочерний участок трассировки или
связь, а не случайный несвязанный идентификатор. Не используйте идентификатор трассировки
как секрет или основание авторизации.

Шифрование канала подтверждает сервер, а **взаимный TLS** дополнительно подтверждает
клиента сертификатом. Он полезен для идентичности рабочих нагрузок, но не заменяет
проверку права конкретной операции. Нужны выпуск, ротация, отзыв сертификатов и
наблюдение за сроком их действия.
...
* Отказ отдельных сервисов — норма, система должна быть спроектирована
  с учётом этого (автоматический выключатель, повтор, изоляция ресурсов,
  сервисы без локального состояния).
...
**Основной источник:** [Microservices Мартина Фаулера](https://martinfowler.com/articles/microservices.html).

## Краткий итог

- Отдельный сервис добавляет сетевые отказы и эксплуатационные расходы.
- Граница сервиса оправдана независимым изменением и владением данными.
- Повторная доставка требует идемпотентности и совместимого развития договоров.

Дальше: изучите [договоры и обработку ошибок gRPC](../gRPC/index.md).
