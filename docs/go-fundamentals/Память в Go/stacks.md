...
**После главы:** вы сможете отличить стек горутины от системного стека потока,
объяснить рост копированием и исследовать рост памяти из-за числа горутин.
...
Минимум `stackMin` в Go 1.27 равен 2 КиБ. На `linux/amd64` минимальный
выделяемый `fixedStack` также равен 2 КиБ. На некоторых системах добавляется
место для обработки сигналов, после чего размер округляется до степени двойки.

Фраза «каждая новая горутина всегда получает 2 КиБ» для Go 1.27 уже неточна.
...
особых условий используются порядки 2, 4, 8 и 16 КиБ; точное число порядков
...
- 1 ГиБ на 64-разрядных системах;
- 250 МиБ на 32-разрядных системах.
...
- Минимум стека равен 2 КиБ, но начальный размер новых горутин может адаптироваться.
- Стек растёт и уменьшается копированием; его освобождение не обязано сразу
  уменьшать RSS.

Дальше: примените метрики и профили в главе
