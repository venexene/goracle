...
> **Проверено:** Go 1.27.0 · **Уровень:** младший разработчик — углублённый

**Перед чтением:** [виртуальная память процесса](virtual-memory.md), указатели и
основы сборки мусора. **После главы:** вы сможете проследить путь выделения в
куче через классы размера, `mcache`, `mcentral` и `mheap` и объяснить, почему
освобождение объекта не обязано сразу уменьшать RSS.
...
|---|---:|
| большинство 64-разрядных целей | 64 МиБ |
| Windows, iOS/arm64 и 32-разрядные цели | 4 МиБ |
| WebAssembly | 512 КиБ |

Внутренняя страница аллокатора равна 8 КиБ. Не путайте её с физической страницей
ОС, размер которой хранится отдельно и зависит от платформы.

![Арена кучи разделена на внутренние страницы размером 8 КиБ](assets/Ga0SouWoXZfjWcpCgw_K6pJ_W09AgDCHSICE47KAgJk=.png)

Арена не является одной аллокацией пользователя и не означает 64 МиБ
...
![Полные и частично заполненные спаны переходят между наборами mcentral](assets/A3eb39yLUJNNLSs1iOMkQQIetU5svhBs05sutDiUy-4=.png)
...
сканируемых объектов данные хранятся в битовой карте спана; для более крупных
используется заголовок перед объектом.

![Битовая карта спана и заголовок выделения описывают указательные поля объектов](assets/LgD0BAxUYTaumtpuqPbkFLk8IVRlNDKGYWgmmTtdAsg=.png)
...
Номинальная граница таблицы классов — 32 КиБ. Заголовок сканируемого объекта и
конкретная функция выделения делают проверку около границы тоньше, поэтому не
используйте 32 КиБ как прикладной порог оптимизации.
...
[«Сборщик мусора»](../Сбощик%20мусора%20в%20Go/garbage-collector.md).
...
Дальше: изучите [стеки горутин](stacks.md) или примените знания в
