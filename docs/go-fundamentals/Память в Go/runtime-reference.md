# Справочник по внутреннему устройству памяти Go 1.27
...
> **Зачем это Go-разработчику.** Страничная организация позволяет среде
> выполнения резервировать большие диапазоны виртуальных адресов, не занимая
> столько же физической памяти до использования страниц. Размер арены кучи —
> деталь реализации, зависящая от архитектуры; 64 МиБ не универсальны.
...
В Go 1.27 исходный предел `debug.SetMaxStack` равен **1 ГиБ на 64-битных** и
**250 МиБ на 32-битных** системах. `debug.SetMaxStack` меняет предел будущего
роста стека и прежде всего ограничивает ущерб от бесконечной рекурсии; система
может накладывать меньший предел. Это общая для процесса защитная настройка, а
не обычный способ улучшения производительности.
...
`ReadMemStats` возвращает согласованный снимок и требует более тяжёлой
координации со средой выполнения, чем чтение отдельных метрик. Для постоянной
телеметрии предпочитайте показатели из `runtime/metrics`, а `MemStats` и профили
кучи используйте для редкой диагностики.

### `runtime/metrics` и границы ограничения памяти

Для панелей наблюдения особенно полезны `/memory/classes/*`, `/gc/heap/*`,
`/gc/cycles/*` и `/sched/*`. Мягкое ограничение `GOMEMLIMIT` или
`debug.SetMemoryLimit` нацелено примерно на `MemStats.Sys -
MemStats.HeapReleased`. Оно не ограничивает RSS и само по себе не защищает от
нехватки памяти; в него не входят двоичный файл, выделения C, память ядра и
отображения через `syscall.Mmap`. Оставляйте запас до ограничения контейнера.
...
`runtime/trace` позволяет записать трассировку выполнения, включающую все аллокации и события GC:
...
> **Зачем это Go-разработчику.** Без инструментов знания об устройстве памяти остаются теорией. `pprof`, `runtime.MemStats` и трассировки `GODEBUG` помогают диагностировать утечки, избыточные выделения и аномалии сборщика в рабочей среде.

.

## 25. Память процесса и время жизни ресурсов

Профиль кучи описывает объекты Go, но не равен RSS процесса. RSS дополнительно
включает стеки, код, отображённые файлы, память библиотек через cgo и страницы,
которые среда выполнения ещё не вернула операционной системе. В контейнере сравнивайте
метрики Go с текущим потреблением и пределом cgroup; аварийное завершение по памяти
может произойти раньше, чем график `HeapAlloc` выглядит критичным.

Полезные величины `runtime/metrics`:

- `/gc/heap/live:bytes` — объём живой кучи после последней сборки;
- `/memory/classes/heap/objects:bytes` — память объектов кучи;
- `/memory/classes/heap/released:bytes` — память кучи, возвращённая системе;
- `/gc/heap/allocs:bytes` — накопленный объём выделений, полезный как скорость изменения.

Профиль `allocs` отвечает, где память выделялась, а `heap` — что осталось живым на
момент снимка. Рост RSS при стабильной живой куче направляет расследование к стекам,
cgo, отображениям и возврату страниц, а не автоматически к «утечке Go-объектов».

`runtime.AddCleanup` в Go 1.24+ позволяет связать очистку с недостижимостью объекта,
а пакет `weak` — хранить слабую ссылку. Они не заменяют явный `Close`: время запуска
очистки не гарантировано, порядок не определён, а завершение процесса может произойти
раньше. `runtime.SetFinalizer` имеет ещё более сложные ограничения и нужен прежде
всего для совместимости с существующим низкоуровневым кодом.

Различайте виды утечек:

- живые Go-объекты удерживаются ссылкой из кеша или замыкания;
- горутина навсегда ждёт канал, блокировку или ввод-вывод;
- файл, сокет или дескриптор не закрыт;
- память выделена внешней библиотекой;
- свободные страницы ещё учитываются в RSS.
...
* pprof, GODEBUG и trace — основные инструменты диагностики памяти в рабочей среде.
