...
> **Проверено:** Go 1.27
>
> **Перед чтением:** [модель сборщика](garbage-collector.md), профилирование и метрики процесса.
>
> **После чтения:** вы сможете связать симптом с метриками, профилями и `gctrace` и провести ограниченный эксперимент.
>
...
Сборка делает ячейки доступными аллокатору, а **очиститель страниц (scavanger)** возвращает ОС
...
- Живую кучу, темп выделений, RSS и мягкий предел памяти анализируют раздельно.
- Профили, метрики и `gctrace` дополняют друг друга.
- `GOGC` и `GOMEMLIMIT` изменяют только в воспроизводимом эксперименте.
- Ручной запуск сборки не заменяет поиск удерживающих ссылок и лишних выделений.

## Продолжение

Проведите ограниченный эксперимент из [практикума](../../practice.md#garbage-collector), затем при необходимости изучите [Green Tea](green-tea.md).
