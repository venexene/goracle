...
**Перед чтением:** типы, срезы и сравнимость значений. **После главы:** вы сможете
выбрать допустимый ключ, отличить отсутствие от нулевого значения и организовать
безопасный конкурентный доступ. [Задания и самопроверка](../../practice.md#maps).
...
* **Актуальную** (Go 1.24+) — на основе **Swiss Table** с открытой адресацией,
  управляющим словом и расширяемым хешированием. Применение векторных инструкций
  зависит от архитектуры и версии среды выполнения.
...
Встроенная функция `clear(m)` удаляет все записи, сохраняя само значение мапы.
После `clear` длина равна нулю, но открытый контракт не обещает немедленный
возврат всей занятой памяти операционной системе. Если большая структура больше
не нужна, можно присвоить `nil` или создать новую мапу и позволить сборщику
удалить старую.
...
Цикл `for-range` по мапе **не имеет определённого порядка**. Два прохода могут
совпасть или различаться. Если нужен устойчивый результат, соберите ключи в срез
и явно отсортируйте.

### Потокобезопасность

Обычные мапы не поддерживают несинхронизированное чтение с записью или две записи
из разных горутин: это гонка данных и некорректная программа. Среда выполнения
часто завершает процесс сообщением `concurrent map ...`, но это диагностика, а
не гарантированный детектор. Для общего доступа задайте владельца, примените
блокировку или подходящий конкурентный контейнер; запускайте тесты с `-race`.
...
До Go 1.24 стандартный компилятор представлял значение мапы через ссылку на
внутреннюю структуру `hmap`. Сам тип `map[K]V` не является типом указателя, а
раскладка ниже — историческая деталь реализации:
...
func replaceMap(m2 map[string]int) {
...
Управляющее слово позволяет получить битовую маску подходящих ячеек несколькими
операциями над машинным словом. Реализация может применять векторные инструкции
на некоторых архитектурах, но открытый API не обещает определённый набор команд.
...
Так одновременно отбрасывается несколько неподходящих ячеек и сокращается число
полных сравнений ключей. Формулировка «восемь сравнений одной инструкцией» не
переносима между архитектурами и версиями среды выполнения.
...
Компилятор и среда выполнения выбирают функцию хеширования, согласованную с
правилами равенства типа ключа. Для некоторых представлений используются
внутренние функции `memhash`, но составные ключи нельзя описывать как
«рекурсивное хеширование всех указателей»: хеш учитывает значения по семантике
типа, а не адреса объектов, на которые указывают поля.
...
Пакет **`hash/maphash`** предоставляет отдельный хеш с начальным случайным
значением для байтов и строк. Он не раскрывает внутреннюю функцию мапы и не
гарантирует совпадение результатов со средой выполнения.
...
* **Предсказуемая хвостовая задержка.** Ограничение в 128 групп на таблицу предотвращает катастровозможностейески долгие операции роста.
...
* **Длинные хвостовые задержки при некоторых вставках.** Отдельная вставка может
  быть значительно медленнее средней. Если задержка важна, измеряйте нужные типы
  ключей и значений, размер мапы и целевую архитектуру.
...
Указывайте подсказку размера, если он известен и достаточно велик, чтобы лишние
расширения были значимы:
...
`struct{}` — обычный маркер присутствия без полезных данных. Реальный объём мапы
зависит от раскладки групп, ключей, распределителя памяти и версии среды
выполнения; измеряйте его отдельно.
...
* Выбирайте тип ключа по смыслу и устойчивости API. Числовое перечисление может
  быть уместно в закрытой модели, но не заменяйте понятный строковый идентификатор
  только ради предположения о скорости без профиля.

### Потокобезопасный доступ

Для конкурентного доступа к мапе из нескольких горутин используйте:

* **`sync.Mutex` или `sync.RWMutex` с мапой** — простой типизированный вариант.
  `RWMutex` может помочь при долгих конкурентных чтениях, но на коротких
  критических участках обычный `Mutex` нередко быстрее; проверяйте свою нагрузку.
...
> **Зачем это Go-разработчику.** Мапа — мощный инструмент, но не серебряная пуля. Правильный выбор начальной ёмкости, типа ключа и стратегии конкурентного доступа напрямую влияет на производительность и стабильность рабочего кода.
...
- Чтение отсутствующего ключа возвращает нулевое значение, а наличие показывает второй результат.
- Порядок обхода мапы не определён, и на него нельзя опираться в договоре программы.
- Обычная мапа не допускает несинхронизированного конкурентного изменения.

Дальше: изучите [синхронизацию доступа к общему состоянию](../Синхронизация%20в%20Go/synchronization.md).
