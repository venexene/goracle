...
**Перед чтением:** определённые типы, методы и указатели. **После главы:** вы
сможете определить набор методов, объявить малый интерфейс потребителя и избежать
ошибок с типизированным `nil` и несравнимыми значениями. [Задания и самопроверка](../../practice.md#interfaces).
...
В Go используется **неявная статическая реализация интерфейсов**: компилятор
проверяет набор методов типа без ключевого слова `implements`. Это иногда
сравнивают с утиной типизацией, но проверка происходит при компиляции, а не
динамическим поиском метода во время вызова.
...
## 4. Наборы методов значения и указателя

Набор методов типа `T` содержит методы с получателем `T`. Набор методов `*T`
содержит методы с получателем `T` и `*T`. Поэтому значение не удовлетворяет
интерфейсу, если требуемый метод объявлен только для получателя-указателя:
...
var _ Closer = (*File)(nil) // допустимо
// var _ Closer = File{}    // ошибка компиляции
```

При обычном вызове `value.Close()` компилятор иногда автоматически берёт адрес,
если значение адресуемо. Но присваивание интерфейсу проверяется именно по набору
методов и такого преобразования не делает.
...
**Type Assertion** позволяет нам вызывать только те методы, которые существуют у данного конкретного типа. Она возвращает две переменные: значение конкретного типа и флаг, указывающий на успешность преобразования значения интерфейсного типа в конкретный тип. После успешного преобразования мы можем вызывать методы, специвозможностейные для этого конкретного типа.
...
## 10. Сравнение значений интерфейсного типа

Два интерфейсных значения можно сравнить, но операция паникует, если их
совпадающий динамический тип несравним:
...
Это особенно важно для `map[any]V`, устранения повторов и обобщённых функций.
Проверить тип можно через `reflect.Type.Comparable`, но чаще лучше сразу
ограничить область допустимыми сравнимыми типами и не принимать произвольный `any`.

## 11. Устройство интерфейса «под капотом»

Этот раздел показывает упрощённую модель стандартного компилятора и среды
выполнения Go 1.27. Названия `iface`, `eface` и `itab`, число машинных слов и
способы упаковки или девиртуализации — детали реализации, а не контракт языка.
Модель помогает понять типизированный `nil`, но производительность проверяйте
тестом и исходным кодом конкретного тега Go.
...
## 12. Популярные интерфейсы стандартной библиотеки
...
Один из самых часто реализуемых интерфейсов. Пакет `fmt` учитывает `Stringer` во
многих вариантах форматирования, но результат зависит от директивы и приоритета
специальных интерфейсов (`Formatter`, `GoStringer`, `error`). Для обычных
`%v`, `%s` и `Println` метод `String` обычно используется автоматически.
...
Фраза **«принимайте интерфейсы, возвращайте конкретные типы»** — полезная
эвристика, но не правило для каждой функции. Принимайте интерфейс, когда нужны
разные реализации и можно выразить минимальное поведение. Не создавайте его
только ради теста или единственного конкретного типа. Возвращайте интерфейс,
если абстракция сама является устойчивым открытым контрактом (`io.Reader`,
`fs.File`); иначе конкретный результат обычно информативнее.

```go
// Хорошо: принимает интерфейс, возвращает структуру
func NewServer(db Database) *Server { ... }

// Подозрительно без дополнительного контекста: принимает конкретную БД,
// но скрывает результат за несвязанным интерфейсом
func NewServer(db *PostgresDB) Database { ... }

```

Фабрика с несколькими реализациями — нормальный случай возврата интерфейса.
Критерий — полезный контракт для вызывающего кода, а не число конкретных типов.
...
## 14. Обычные и обобщённые интерфейсы

Обычный интерфейс описывает только методы и может использоваться как тип значения:
...
Обобщённый интерфейс может дополнительно содержать элементы типов: объединения
`A | B` и приближение `~T`. Он задаёт множество допустимых типов для параметра:
...
Интерфейс с элементами типов предназначен только для ограничения параметров типа:
его нельзя использовать как тип обычной переменной. `~int` означает все типы,
чей базовый тип — `int`, включая `type UserID int`.

В ограничениях полезны встроенные `any` и `comparable`. `comparable` допускает
типы, значения которых можно сравнивать операторами `==` и `!=`, и поэтому подходит
для ключей обобщённой мапы. Не путайте это с интерфейсным значением `any`: операция
сравнения двух таких значений может вызвать панику, если внутри хранится срез или мапа.

Для извлечения типизированной ошибки Go 1.26+ предоставляет `errors.AsType[E](err)`.
Подробные правила дерева ошибок и пример приведены в главе [«Ошибки в Go»](../Ошибки%20в%20Go/errors.md#errors-as-type).

### Самопроверка

1. Почему `bytes.Buffer` удовлетворяет `io.Writer` только через указатель? Подтвердите ответ проверками `var _`.
2. Создайте ограничение для определённых целочисленных типов и функцию поиска максимума.
3. Верните типизированный нулевой указатель как `error`, объясните результат `err == nil` и исправьте функцию.
4. Продемонстрируйте безопасное сравнение двух значений `any`, динамические значения которых заранее неизвестны.
...
- Тип реализует интерфейс неявно своим набором методов.
- Интерфейс с динамическим типом и типизированным `nil` не равен пустому интерфейсу.
- Интерфейс должен отражать потребность вызывающего кода, а не полный набор методов реализации.

Дальше: примените интерфейсы к [договорам ошибок](../Ошибки%20в%20Go/errors.md).
