...
**Перед чтением:** типы, указатели и циклы. **После главы:** вы сможете предсказать
длину и ёмкость среза, управлять общим массивом и создавать независимую копию.
[Задания и самопроверка](../../practice.md#slices).
...
Не каждый локальный массив размещается в стеке горутины. Решение принимает
компилятор по анализу времени жизни, размеру объекта, возможности встраивания
функций и другим внутренним ограничениям. `MaxStackVarSize` — нестабильная
константа компилятора, а прежний порог 10 МБ неверен. Не проектируйте API вокруг
конкретной цифры: проверяйте код командой `go build -gcflags=all=-m=2` на целевой
версии Go.
...
**Срез** — это представление части массива. Полная форма записывается как
`[low:high:max]`; также используются `[low:high]`, `[:high]`, `[low:]` и `[:]`.
`low` — индекс первого включаемого элемента, `high` — исключающая граница длины,
`max` — исключающая граница доступной ёмкости. Третий индекс не равен итоговой
ёмкости: `len = high-low`, `cap = max-low`.

Компилятор или среда выполнения проверяет индексы `low`, `high` и `max`.
...
* `max` — ёмкость исходного среза или длина исходного массива.

Новая длина равна `high-low`, а ёмкость — `max-low` либо `cap(source)-low`, если
третий индекс отсутствует.
...
Если `low == len(source)`, длина результата равна нулю, но ёмкость может быть
ненулевой: у исходного среза возможно `cap(source) > len(source)`. Например,
`make([]int, 2, 5)[2:]` имеет `len=0`, `cap=3`.
...
Выражение после `range`, имеющее тип массива, вычисляется как значение, поэтому
семантически цикл проходит по копии массива. Компилятор может устранить
фактическое копирование, если результат не изменится. Переменная итерации `v`
содержит значение очередного элемента и не является ссылкой на него.
...
Для ненулевого пустого среза `unsafe.SliceData` может вернуть ненулевой указатель
с неустановленным значением. Использование внутреннего символа вроде
`runtime.zerobase` — деталь реализации: на уникальность или конкретный адрес
полагаться нельзя.
...
emptySlice := []int{}         // пустой ненулевой срез: len=0, cap=0
emptyMake := make([]int, 0)   // обычно пустой ненулевой срез: len=0, cap=0
...
```go
func main() {
    a := [3]int{1, 2, 3} // кандидат на размещение в стеке
...
В таком небольшом примере компилятор обычно может оставить базовый массив в
стеке. Печать адреса и оптимизации способны повлиять на решение, поэтому
расположение нельзя доказывать близостью адресов; используйте диагностику
компилятора.

### Случай 2: `append` меняет базовый массив

```go
func main() {
    s := make([]int, 2)
    s = append(s, 1, 2, 3, 4, 5) // превышает cap → новый базовый массив
    fmt.Printf("&s[0]: %p\n", &s[0])
}

```

При превышении ёмкости `append` выделяет новый базовый массив и копирует элементы.
Но новый массив не обязан находиться в куче: если компилятор видит размер и
значение не покидает область вызова, он может разместить память в стеке.

Если ожидаемый размер известен, разумная начальная ёмкость сокращает повторные
выделения и копирования. Не задавайте произвольно огромный запас: он удерживает
лишнюю память. Для небольших срезов естественный рост может быть дешевле оценки.
...
Большие локальные объекты компилятор может переместить в кучу даже при известном
размере, чтобы не раздувать стек горутины. Пороги меняются между версиями и не
являются API; цифра 64 КиБ не универсальна. Сравните диагностику для `sliceA` и
`sliceB` на своей версии Go.

Базовый массив с размером, известным лишь во время выполнения, стандартный
компилятор обычно размещает в куче:

```go
func main() {
    n := 100000
    s := make([]int, n) // размер известен только во время выполнения
    fmt.Printf("&s[0]: %p\n", &s[0])
}

```

Это свойство текущей стратегии компилятора, а не различие семантики среза.
Прикладной код не должен зависеть от физического размещения памяти.
...
func processData() []int {
    // Большой массив в памяти
    largeArray := make([]int, 1000000) // 1 млн элементов

    // Мы берем только маленький срез
    smallSlice := largeArray[0:10] // всего 10 элементов
...
func main() {
    ...
    // Проблема: весь largeArray (1 млн элементов) остается в памяти,
    // потому что result ссылается на него через smallSlice
}
...
| Операция | Массив | Срез |
|---|---|---|
| Индексация (`a[i]`) | $O(1)$ | $O(1)$ |
| `len(s)` | $O(1)$ | $O(1)$ |
| `cap(s)` | $O(1)$ | $O(1)$ |
| Итерация (`for range`) | $O(n)$ | $O(n)$ |
| Сравнение (`==`) | $O(n)$ | не определён |

**Операции создания:**

| Операция | Сложность | Примечание |
|---|---|---|
| Литерал `a := [N]T{...}` | $O(n)$ | компилятор инициализирует |
| `make([]T, len, cap)` | $O(n)$ | элементы — zero values |
| `var s []T` (nil) | $O(1)$ | только заголовок |
| Срез из массива `a[i:j]` | $O(1)$ | не копирует данные |
| Срез из среза `s[i:j]` | $O(1)$ | не копирует данные |

**Операции добавления и удаления для среза:**

| Операция | Сложность | Примечание |
|---|---|---|
| `append(s, v)` — без роста | $O(1)$ аморт. | ёмкости хватает |
| `append(s, v)` — с ростом | $O(n)$ | новый массив + копирование |
| Вставка в середину | $O(n)$ | сдвиг элементов |
| Удаление из середины | $O(n)$ | сдвиг элементов |
| Удаление с конца | $O(1)$ | только `len` меняется |

**Операции копирования и преобразования:**

| Операция | Сложность | Примечание |
|---|---|---|
| `copy(dst, src)` | $O(\min(len(dst), len(src)))$ | побайтовое копирование |
| Срез → срез (тот же тип) | $O(n)$ | создание копии |
| Массив → срез (`a[:]`) | $O(1)$ | только заголовок |
| Срез → массив (Go 1.17+) | $O(n)$ | копирование в новый массив |
...
| `Compare(s1, s2)`        | Лексикогравозможностейеское сравнение         |
...
* [Срезы: использование и устройство](https://go.dev/blog/slices-intro) — официальный блог Go

## Краткий итог

- Массив содержит элементы, а срез описывает участок массива длиной и ёмкостью.
- Копирование среза не копирует его элементы и может сохранить общее изменяемое хранилище.
- `append` вправе заменить массив, поэтому код не должен полагаться на конкретную схему роста.

Дальше: сравните свойства среза с [мапой](../Мапы%20в%20Go/maps.md).
