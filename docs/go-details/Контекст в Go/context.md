...
**Перед чтением:** горутины, каналы и ошибки. **После главы:** вы сможете провести
отмену и дедлайн через слои, сохранить причину отмены и ограничить отделённую
работу. [Задания и самопроверка](../../practice.md#context).
...
- Контекст несёт отмену, срок и данные запроса через границы вызовов.
- Создавшая контекст сторона вызывает функцию отмены и тем самым освобождает связанные ресурсы.
- Контекст передают первым параметром и не сохраняют в структуре без особой причины.

Дальше: примените отмену к [каналам](../Каналы%20в%20Go/channels.md) и [HTTP-службе](../../architecture/HTTP/http.md).
