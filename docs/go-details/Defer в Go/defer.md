...
**Перед чтением:** функции, возвращаемые значения и ошибки. **После главы:** вы
сможете предсказать порядок отложенных вызовов, освободить частично полученные
ресурсы и сохранить ошибку закрытия. [Задания и самопроверка](../../practice.md#defer).
...
**Основные источники:** [отложенные вызовы в спецификации Go](https://go.dev/ref/spec#Defer_statements) и [разбор `defer`, `panic` и `recover` в блоге Go](https://go.dev/blog/defer-panic-and-recover).

## Краткий итог

- Аргументы отложенного вызова вычисляются при выполнении `defer`, а вызовы идут в обратном порядке.
- Освобождение ресурса ставят сразу после успешного получения этого ресурса.
- Ошибку закрытия сохраняют, когда она влияет на результат операции.

Дальше: объедините ошибки операции и освобождения по правилам главы [«Ошибки в Go»](../Ошибки%20в%20Go/errors.md).
