...
> **Зачем это Go-разработчику.** Go помогает строить конкурентный дизайн через горутины и каналы, скрывая низкоуровневые детали. Но понимание разницы между Concurrency и Parallelism напрямую влияет на решение *когда* применять параллелизм и *какой* выигрыш от него ожидать.
...
> **Зачем это Go-разработчику.** В Go асинхронность достигается через горутины и каналы, а не через callback-и или async/await. Горутина, заблокировавшаяся на чтении из канала или сетевом вызове, не занимает поток ОС — рантайм Go переключается на другую горутину.
