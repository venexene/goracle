...
**Перед чтением:** пакеты, функции, ошибки и интерфейсы. **После главы:** вы
сможете выбрать уровень проверки, написать табличный тест, запустить фаззинг,
замер и детектор гонок. [Задания и самопроверка](../../practice.md#testing).
...
Флаги проще использовать как справочник, а не искать по всей главе:

| Флаг | Назначение |
|---|---|
| `-run regexp` | Запустить подходящие тесты и подтесты |
| `-skip regexp` | Пропустить подходящие тесты |
| `-list regexp` | Показать подходящие тесты, тесты производительности и примеры без запуска |
| `-v` | Показать подробный вывод |
| `-short` | Включить короткий режим; сам тест решает, что пропустить через `testing.Short()` |
| `-failfast` | Остановить запуск новых тестов после первой ошибки |
| `-count=n` | Повторить запуск; `-count=1` не использует сохранённый результат тестов |
| `-shuffle=on` | Перемешать порядок тестов верхнего уровня и тестов производительности |
| `-parallel=n` | Ограничить число одновременно исполняемых параллельных тестов |
| `-timeout=d` | Прервать тесты пакета после общего срока |
| `-race` | Включить динамический детектор гонок данных на поддерживаемых платформах |
| `-cover` | Посчитать покрытие текущего пакета |
| `-coverprofile=file` | Записать профиль покрытия |
| `-covermode=set|count|atomic` | Выбрать режим подсчёта покрытия |
| `-coverpkg=pattern` | Считать покрытие дополнительных пакетов |
| `-bench=regexp` | Запустить тесты производительности |
| `-benchmem` | Показать байты и выделения памяти на операцию |
| `-fuzz=regexp` | Запустить цель фаззинга |
| `-json` | Вывести события в JSON, удобном для непрерывной проверки и инструментов |
| `-cpu=list` | Повторить тесты с указанными значениями `GOMAXPROCS` |
| `-vet=list` | Настроить проверки `go vet`, запускаемые вместе с тестами |
| `-fullpath` | Показывать полные пути в именах файлов ошибок |
| `-args ...` | Передать оставшиеся аргументы исполняемому файлу тестов |

Флаги до списка пакетов обрабатывает `go test`; после `-args` они передаются
исполняемому файлу тестов без дополнительного разбора командой `go`.

### Возможности современного `testing`

- `t.Context()` возвращает контекст, который отменяется перед запуском функций `Cleanup`;
- `t.TempDir()`, `t.Setenv()` и `t.Chdir()` упрощают изоляцию файловой системы и окружения; методы, меняющие общее состояние процесса, нельзя совмещать с `t.Parallel()`;
- `t.Attr()` добавляет к тесту пару «ключ — значение», а `t.Output()` даёт поток вывода, связанный с журналом теста;
- пакет `testing/synctest` запускает конкурентный код в изолированном окружении с управляемым временем; `synctest.Wait()` ждёт, пока остальные горутины внутри него устойчиво заблокируются.

`synctest` не предназначен для реальной сети и внешних процессов. Зависимости такого кода нужно заменять подконтрольными реализациями.
...
func TestMain(m *testing.M) {
    ...
    if err := testDB.PingContext(context.Background()); err != nil {
        log.Fatal(err)
    }
    ...
    if _, err := testDB.Exec("TRUNCATE users"); err != nil {
        log.Printf("не удалось очистить тестовую базу: %v", err)
    }
    if err := testDB.Close(); err != nil {
        log.Printf("не удалось закрыть тестовую базу: %v", err)
    }
}
```

`m.Run()` возвращает код завершения. В современном Go `TestMain` может просто вернуть управление: созданная `go test` функция-обёртка передаст результат последнего `m.Run()` в `os.Exit`. Это позволяет выполнить обычные `defer`. Если вызвать `os.Exit(code)` самостоятельно, отложенные функции не выполнятся, поэтому очистку нужно завершить до этого вызова.
...
Бенчмарк — функция с сигнатурой `func BenchmarkXxx(b *testing.B)` в `_test.go`. Go запускает её многократно до получения устойчивого измерения. Запуск: `go test -bench=.` (все бенчмарки) или `go test -bench=BenchmarkFoo`.
...
`b.Loop`, добавленный в Go 1.24, управляет измеряемым циклом и автоматически не включает подготовку и очистку вокруг цикла в измеряемое время. Старый цикл `for i := 0; i < b.N; i++` по-прежнему поддерживается и часто встречается в существующем коде. Вывод:
...
Пакет `net/http/httptest` — инструменты для тестирования HTTP. Подробный разбор — в [статье про HTTP](../../architecture/HTTP/http.md) (раздел 15). Здесь — краткий обзор для контекста тестирования.
...
- `-race` — детектор гонок данных; обычно его запускают в системе непрерывной проверки отдельным заданием, потому что он заметно замедляет тесты
...
- **Необоснованного усложнения проверок.** Стандартных `if` часто достаточно. Библиотека проверок допустима, если команда принимает эту зависимость, а сообщения об ошибках остаются понятными.
...
> **Зачем это Go-разработчику.** Тесты — это код, который нужно поддерживать. Хорошие тесты документируют поведение, ловят регрессии и позволяют менять код увереннее. Начинайте с проверки публичного поведения, применяйте табличную форму там, где действительно повторяется один сценарий, и выбирайте способ проверки по ясности результата, а не по догме.
...
- Тест проверяет наблюдаемое поведение, а не повторяет внутреннее устройство функции.
- Табличные случаи полезны, когда разделяют одну подготовку и сохраняют понятную причину сбоя.
- `-race`, `-shuffle`, `-count`, покрытие и тесты производительности отвечают на разные вопросы.

Дальше: примените уровни проверки к [сквозной HTTP-службе](../../practice.md#capstone).
