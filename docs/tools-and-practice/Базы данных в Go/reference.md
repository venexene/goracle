...
Эта статья идёт от основ: сначала разбирается, что такое реляционная база данных и SQL, затем ACID и транзакции, и только потом — Go-специвозможностейные инструменты. Такой порядок потому, что нельзя эффективно использовать `database/sql`, не понимая, что происходит на стороне базы данных.
...
Правило: если данные имеют чёткую структуру и связи между собой (а большинство бизнес-данных именно такие) — берите реляционную БД. NoSQL — для специвозможностейеских случаев: логи, кеш, временные ряды.
...
| `verify-full` | SSL + проверка сертификата (рабочая среда) |

> В рабочем окружении всегда используйте `sslmode=verify-full` или хотя бы `require`. `disable` допустим только на `localhost`.
...
`sql.Open` создаёт дескриптор пула и обычно не устанавливает соединение. Чтобы
проверить доступность с конечным сроком, вызовите `db.PingContext(ctx)`.
...
> Для сервиса, который не может работать без БД, проверка `PingContext` при старте
> позволяет завершиться с понятной ошибкой. Если допустим запуск без БД, выберите
> явную политику повторов и состояния готовности вместо безусловного отказа.
...
> Для серверного приложения обычно задают `SetMaxOpenConns` с учётом общего
> лимита СУБД, числа экземпляров сервиса и запаса для служебных подключений.
> Значение подбирают по `DB.Stats`, задержке ожидания и нагрузочному испытанию.
...
Ключевые моменты:
- `rows.Close()` позволяет раньше вернуть соединение в пул. После полного чтения
  `Rows` закрывается автоматически, но явный `defer` защищает ранние выходы.
...
Директива `//go:embed migrations/*.sql` встраивает SQL-файлы в исполняемый файл при компиляции. Каталог `migrations/` не нужно поставлять рядом с программой: все миграции уже находятся внутри неё.
...
> Всегда тестируйте `up` и `down` локально перед применением в рабочей среде.

> **Зачем это Go-разработчику.** Схема должна изменяться воспроизводимо и
> проверяться до развёртывания. Обратная миграция полезна, когда она безопасна,
> но удаление данных или столбца часто необратимо. Для таких изменений применяют
> поэтапную схему «расширить — перенести — переключить — удалить», резервную
> копию и исправление следующей миграцией.
...
> **Зачем это Go-разработчику.** `sqlx` сохраняет явный SQL и сокращает ручное
> сканирование. Это один из вариантов, а не универсальная середина: `database/sql`
> уменьшает число зависимостей, `pgx` даёт возможности PostgreSQL, а `sqlc`
> переносит проверку запросов в генерацию. Выбор зависит от СУБД, требований к
> типам и готовности использовать генерацию кода.
...
[`pgx`](https://github.com/jackc/pgx) — это не просто драйвер, а полноценный клиент PostgreSQL на чистом Go. В отличие от `database/sql` + `pgx/stdlib`, нативный `pgx` даёт прямой доступ к PostgreSQL-специвозможностейным возможностям: `COPY`, `LISTEN`/`NOTIFY`, массивы, JSONB, уведомления.
...
`pgxpool.New` разбирает конфигурацию и создаёт пул, но не гарантирует немедленное
соединение. Для проверки доступности вызовите `Ping(ctx)` и задайте срок в
контексте.
...
### PostgreSQL-специвозможностейные типы
...
`AutoMigrate` НЕ удаляет колонки и не меняет типы — только добавляет новые колонки и индексы. Для миграций в рабочей среде используйте `golang-migrate`, а `AutoMigrate` — для разработки и тестов.
...
- `AutoMigrate` в рабочей среде — риск потерять данные
...
Сервис зависит от интерфейса, а не от конкретной реализации. В рабочей среде передаётся `NewUserRepository(db)`, в тестах — имитация.
...
3. **SSL/TLS между приложением и БД.** `sslmode=verify-full` в рабочей среде (раздел 5). Без него SQL-запросы идут открытым текстом.
...
Медленные запросы убивают приложение быстрее, чем отсутствие возможностей. Три кита производительности БД: индексы, анализ плана запроса и мониторинг.
...
> **Зачем это Go-разработчику.** Индексы и `EXPLAIN ANALYZE` — не «забота DBA». В стартапе DBA нет — есть вы. Медленный запрос в рабочей среде обнаруживается не глазами, а через `pg_stat_statements` и алерты. N+1 — самая частая причина деградации под нагрузкой: на 10 пользователях незаметно, на 10000 — отказ. Индекс на поле, по которому ищете, — первое, что нужно сделать перед выкаткой возможностейи.

***

<a id="database-testing"></a>
...
Плюсы:
- **Настоящая БД** — точь-в-точь как в рабочей среде, включая особенности диалекта
...
> **Зачем это Go-разработчику.** Тестирование с БД — не опция, а требование. Миграция с опечаткой в имени колонки не упадёт при компиляции — только в рантайме. Транзакционные тесты дают баланс скорости и достоверности: одна БД на пакет, откат после каждого теста. Testcontainers — золотой стандарт для интеграционных тестов: «в CI точно так же, как в рабочей среде». Начните с транзакционных тестов для SQL, добавьте testcontainers для миграций и сложных сценариев.
...
Минусы:
- Не PostgreSQL — не проверить специвозможностейные для PG возможностейи (JSONB, `COPY`, `LISTEN/NOTIFY`)
- Нет проверки реального диалекта — можно случайно использовать синтаксис PostgreSQL, который в рабочей среде упадёт
...
## 18. Отказы, изменения схемы и динамический SQL

Уровень изоляции выбирают по аномалиям, которые нельзя допустить, а не по названию
«самый строгий». Взаимная блокировка и ошибка сериализации означают, что всю
транзакцию нужно откатить; повтор допустим только для классифицированной временной
ошибки, с новым `BeginTx`, конечным числом попыток и идемпотентной бизнес-операцией.
Нарушение ограничения уникальности и синтаксическая ошибка повтором не исправляются.

Параметры SQL заменяют только значения. Имя таблицы, столбца или направление сортировки
нельзя передать через `$1`; допустимые идентификаторы выбирают из белого списка:

```go
columns := map[string]string{"created": "created_at", "name": "display_name"}
column, ok := columns[input.Sort]
if !ok {
    return errors.New("неподдерживаемая сортировка")
}
query := "SELECT id, display_name FROM users ORDER BY " + column
```

Для изменения схемы без остановки сначала расширьте её совместимым полем, выпустите
код двойного чтения или записи, перенесите данные, переключите чтение и только потом
удалите старое поле. Перед опасной миграцией проверьте резервную копию восстановлением:
сам факт успешной команды копирования ещё не доказывает возможность возврата.

N+1 обнаруживают не только в ORM: считайте запросы на один входящий запрос, изучайте
`pg_stat_statements` и планы. Исправления — соединение таблиц, пакетная загрузка или
предварительная выборка. В ORM передавайте объект транзакции во все связанные вызовы;
обращение к исходному `db` внутри функции незаметно выводит часть изменений из
транзакции.

### Самопроверка

1. Напишите классификатор ошибок PostgreSQL для повтора транзакции и тест таблицей.
2. Реализуйте безопасный выбор столбца и направления сортировки из белых списков.
3. Составьте последовательность выпуска для переименования обязательного столбца без остановки.
4. Добавьте счётчик SQL-вызовов в тест и поймайте N+1.
