...
**Перед чтением:** двоичная запись чисел и простая программа на Go. **После главы:**
вы сможете связать выполнение программы с процессором, памятью, системными вызовами
и планированием операционной системы. [Задания и самопроверка](../../practice.md#systems).
...
**Шаг 2.** После нажатия Enter оболочка просит ядро запустить `hello`. Ядро
создаёт процесс и отображает в его виртуальное адресное пространство сегменты
исполняемого файла. Нужные страницы могут загружаться по требованию; конкретный
путь данных и использование прямого доступа к памяти зависят от устройства,
драйвера и файлового кэша.

![](assets/nMHuceLOlLqwnyGxsxQKzBEjrxF2FzQo5rGABwbDdhI=.png)

**Шаг 3.** Процессор выполняет инструкции программы. Для вывода строка передаётся
системному вызову записи в стандартный вывод. Дальнейший путь проходит через
ядро и, например, терминал; программа не копирует байты из регистров прямо на
дисплей.

![](assets/OCIVjdO6cZno2yrGodyvoz93izcYAfzYE031_cVXDAs=.png)

> **Зачем это Go-разработчику.** Файловый `Read` может заблокировать поток ОС;
> планировщик Go обычно позволяет другим горутинам продолжить работу на других
> потоках. Сетевой опросчик использует неблокирующие возможности ОС для сети, но
> из этого не следует, что любая операция ввода-вывода Go неблокирующая.
...
**Рабочий набор** — набор активных страниц, с которыми программа работает в данный момент. Если рабочий набор помещается в DRAM — всё работает быстро. Если нет — возникает **пробуксовка (thrashing)**: страницы непрерывно загружаются и выгружаются, производительность падает катастровозможностейески.
...
* **Сквозная запись** — данные обновляются в кэше и сразу передаются на следующий
  уровень иерархии памяти.
* **Обратная запись** — сначала изменяется только строка кэша, которая помечается
  как изменённая. На следующий уровень иерархии она записывается позже, например
  при вытеснении. Для кэша процессора следующим уровнем обычно является другой
  кэш или оперативная память, а не диск.
...
Процессы изолированы: у каждого своё адресное пространство. Для обмена данными
нужны механизмы **межпроцессного взаимодействия**: каналы ОС, очереди сообщений,
разделяемая память и сокеты. Их стоимость различается; утверждать, что любой
такой обмен обязательно медленный, нельзя.
...
> **Зачем это Go-разработчику.** Горутины мультиплексируются на потоки ОС.
> Переключение между ними обычно дешевле переключения потоков ОС, но фиксированной
> цены в наносекундах нет. Каналы Go передают данные между горутинами одного
> процесса и обеспечивают синхронизацию; это не механизм межпроцессного обмена.
...
* **Каналы** — для небуферизованного канала отправка синхронизирована до
  завершения соответствующего получения. Для буферизованных каналов точные
  гарантии зависят от ёмкости и описаны в модели памяти Go.
...
* **`sync/atomic`** — атомарные операции над отдельными значениями. Они устраняют
  гонку только для правильно построенного протокола; несколько атомарных полей
  сами по себе не делают составной инвариант атомарным.
...
> **Зачем это Go-разработчику.** Детектор гонок (`go test -race`) находит гонки,
> которые проявились во время конкретного запуска; отсутствие отчёта не доказывает
> их отсутствие. Гонка данных в Go — серьёзная ошибка: гарантии для программ без
> гонок к такому коду не применимы, а некоторые гонки могут повредить составные
> значения. Запускайте детектор в непрерывной проверке и исправляйте каждый отчёт.
...
- Процесс, виртуальная память и системные вызовы задают среду выполнения программы Go.
- Порядок байтов и выравнивание важны на границе с двоичными форматами и внешним кодом.
- Ускорение ограничено последовательной частью работы, поэтому сначала измеряют узкое место.

Дальше: изучите [планировщик Go](../../go-fundamentals/Планировщик%20Go/scheduler.md).
