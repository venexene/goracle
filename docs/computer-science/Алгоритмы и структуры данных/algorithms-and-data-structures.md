...
**Перед чтением:** переменные, циклы, функции и срезы. **После главы:** вы сможете
выбрать структуру данных, оценить время и память алгоритма и реализовать основные
графовые алгоритмы. [Задания и самопроверка](../../practice.md#algorithms).
...
| Вставка в середину   | $O(n)$ (поиск и сдвиг)       | $O(n)$ с поиском; $O(1)$ при известном узле |
| Удаление из середины | $O(n)$ (поиск и сдвиг)       | $O(n)$ с поиском; $O(1)$ при известном узле |
...
* При сбалансированных разбиениях стек занимает $O(\log n)$. Наивная реализация
  в худшем случае может потребовать $O(n)$; ограничение глубины или обработка
  меньшей части первой удерживает её логарифмической.
...
Go не предоставляет специвозможностейеского синтаксиса для итераторов. Поэтому следующий/предыдущий элемент можно получить из указателя на любой узел. Эти методы не инвалидируются после добавления/удаления элемента в список:
...
В стандартной библиотеке нет готового битового множества произвольной длины.
Ниже показан пример со сторонним модулем
`github.com/bits-and-blooms/bitset`; его версию следует закрепить в `go.mod`:
...
Для графа $G=(V,E)$ список смежности занимает $O(V+E)$
памяти, а матрица смежности — $O(V^2)$. Список обычно лучше
для разреженного графа, матрица — когда нужна проверка ребра за $O(1)$
и $V^2$ разумно помещается в память.

### Кратчайшие пути: алгоритм Дейкстры

Алгоритм Дейкстры находит кратчайшие расстояния от одной вершины в
графе с **неотрицательными** весами. Для отрицательных рёбер нужен,
например, алгоритм Беллмана — Форда. С двоичной кучей сложность
равна $O((V+E)\log V)$, память — $O(V+E)$.

Шаг расслабления ребра $u \to v$ с весом $w$:

```go
if dist[u] != infinity && dist[u]+w < dist[v] {
    dist[v] = dist[u] + w
    heap.Push(&queue, item{vertex: v, distance: dist[v]})
}
```

В очереди могут оставаться устаревшие записи. После извлечения их
пропускают, если `item.distance != dist[item.vertex]`. Это проще, чем искать
элемент в куче и уменьшать его приоритет.

### Топологическая сортировка

Топологический порядок существует только для ориентированного графа без
циклов. Алгоритм Кана хранит входную степень каждой вершины:

1. поместить в очередь все вершины со степенью 0;
2. извлекать вершину, добавлять её в результат и уменьшать степени её соседей;
3. если обработано меньше $V$ вершин, в графе есть цикл.

Время и память алгоритма — $O(V+E)$.

### Система непересекающихся множеств

Структура **DSU** (также union-find) быстро отвечает, находятся ли два элемента
в одной компоненте. Сжатие путей и объединение по рангу дают
амортизированное время $O(\alpha(n))$ на операцию, что на практике близко к
константному.
...
Минимальное остовное дерево соединяет все вершины связного неориентированного
взвешенного графа с минимальным общим весом.

- **Краскал:** сортирует рёбра по весу и добавляет ребро, если DSU показывает,
  что оно не создаст цикл. Время — $O(E\log E)$.
- **Прим:** растит одно дерево, каждый раз выбирая минимальное ребро к новой
  вершине. С кучей время — $O(E\log V)$.

Если после выбора рёбер получено меньше $V-1$ ребра, исходный граф несвязен
и результатом будет минимальный остовный лес, а не одно дерево.

## 8. Сложности и инструменты Go

| Задача | Лучший или обычный случай | Худший случай | Дополнительная память |
|---|---:|---:|---:|
| Поиск в неупорядоченном срезе | $O(n)$ | $O(n)$ | $O(1)$ |
| Бинарный поиск | $O(1)$ | $O(\log n)$ | $O(1)$ |
| `append` в срез | $O(1)$ амортизированно | $O(n)$ | до $O(n)$ при росте |
| Быстрая сортировка | $O(n\log n)$ в среднем | $O(n^2)$ | $O(\log n)$ в среднем, $O(n)$ в худшем |
| Сортировка слиянием | $O(n\log n)$ | $O(n\log n)$ | $O(n)$ |
| BFS/DFS по списку смежности | $O(V+E)$ | $O(V+E)$ | $O(V)$ помимо графа |
| Дейкстра с кучей | $O((V+E)\log V)$ | $O((V+E)\log V)$ | $O(V+E)$ |
| DSU: `Find`/`Union` | $O(\alpha(n))$ амортизированно | $O(\log n)$ для одной операции | $O(n)$ |

Для обычной сортировки не нужно писать свой алгоритм:
...
`slices.Sort` сортирует встроенно упорядоченные типы, `slices.SortFunc` — значения
по явному сравнению. Функция сравнения должна задавать транзитивный
порядок; иначе результат не имеет смысла.

### Упражнения

1. Реализуйте Дейкстру и верните не только расстояние, но и сам путь. Добавьте тест на недостижимую вершину.
2. Измените топологическую сортировку так, чтобы при цикле она возвращала ошибку.
3. Сравните замером `slices.Sort` и свою сортировку на случайных, уже упорядоченных и обратно упорядоченных данных.
4. Реализуйте Краскала и проверьте отдельно связный и несвязный граф.
...
* **Визуализация алгоритмов**: [VisuAlgo](https://visualgo.net/) помогает увидеть работу алгоритма по шагам.
...
**Основные источники:** документация пакетов [`container/heap`](https://pkg.go.dev/container/heap) и [`sort`](https://pkg.go.dev/sort).

## Краткий итог

- Структуру данных выбирают по требуемым операциям и их ограничениям.
- Оценка сложности описывает рост затрат, но не заменяет измерение реальной программы.
- Корректность алгоритма проверяют обычными, граничными и ошибочными входными данными.

Дальше: примените эти правила к [массивам, срезам и мапам](../../go-details/Массивы%20и%20срезы%20в%20Go/arrays-and-slices.md).
