...
**Перед чтением:** функции, горутины, ошибки и контекст. **После главы:** вы сможете
различать конкурентность и параллелизм, находить гонки и ограничивать число
одновременно выполняемых работ. [Задания и самопроверка](../../practice.md#concurrency).
...
**Параллельное выполнение обычно требует разделить работу на независимые части.**
Конкурентная структура не требует нескольких ядер: на одном ядре планировщик
чередует выполнение готовых задач.

> **Зачем это Go-разработчику.** Конкурентность описывает структуру программы,
> параллелизм — одновременное исполнение на нескольких ядрах. В Go обычный код
> может выглядеть последовательным внутри горутины, а планировщик продолжит
> выполнять другие горутины, когда эта ждёт. Однако блокирующий системный вызов
> способен занять поток ОС, даже если не останавливает всю программу.
...
Такое описание было верно для старых версий Go. Начиная с Go 1.14 среда
выполнения умеет асинхронно вытеснять большинство долгих циклов, поэтому наличие
вызова функции больше не является обязательным условием переключения. Пример не
следует использовать как проверку современного планировщика.
...
> **Зачем это Go-разработчику.** Не вставляйте `runtime.Gosched()` в каждый
> вычислительный цикл. Сначала измерьте задержку. Явная уступка может быть полезна
> в необычном цикле ожидания, но обычно правильнее применить канал, блокировку,
> таймер или контекст отмены.
...
Программа продолжает работать, но для некоторых операций время отклика становится катастровозможностейески большим.
...
2. **Ограниченная история событий.** Детектор хранит конечный журнал обращений
   для каждой горутины. Его объём настраивает `GORACE="history_size=N"` и составляет
   `32K × 2**N` элементов на горутину. Даже большая история не помогает ветке
   программы, которая не выполнилась во время запуска.
...
* [Модель памяти Go](https://go.dev/ref/mem) — официальные гарантии видимости и порядка операций.
* [Детектор гонок Go](https://go.dev/doc/articles/race_detector) — поиск гонок данных и разбор отчётов.

## Краткий итог

- Конкурентность описывает организацию независимых работ, а параллелизм — их одновременное выполнение.
- Доступ к общей изменяемой памяти требует отношения синхронизации.
- Отмена, предел числа задач и ожидание уже запущенной работы входят в договор функции.

Дальше: сравните [каналы](../../go-details/Каналы%20в%20Go/channels.md) и [примитивы синхронизации](../../go-details/Синхронизация%20в%20Go/synchronization.md).
