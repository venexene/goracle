...
**Перед чтением:** байты, процессы и базовая работа с командной строкой. **После
главы:** вы сможете объяснить путь сетевого запроса, задать сроки операций и
правильно выделить сообщения из TCP-потока. [Задания и самопроверка](../../practice.md#networks).
...
Ключевые протоколы: **TCP** (надёжный упорядоченный поток с соединением) и
**UDP** (датаграммы без установки соединения и без гарантии доставки). UDP имеет
меньший заголовок, но не обязан быть быстрее для конкретного приложения.
...
Другой функцией, выполняемой на уровне представлений, является шифрование данных, которое применяется в тех случаях, когда необходимо защитить передаваемую информацию от доступа несанкционированными получателями. На этом уровне существуют и другие подпрограммы, которые сжимают тексты и преобразовывают гравозможностейеские изображения в битовые потоки.

Стандарты уровня представлений также определяют способы представления гравозможностейеских изображений и звука. Для изображений могут использоваться форматы PICT, TIFF или JPEG, а для звука — MPEG, QuickTime и т.д.
...
Многие из этих протоколов работают поверх TCP или UDP. Стандартный порт HTTP —
80; 8080 — распространённый, но не стандартный альтернативный порт.
...
* **Поддержка IPsec** предусмотрена семейством стандартов, но IPv6 сам по себе
  не шифрует трафик и не делает сеть безопасной.
* **Автонастройка**: SLAAC позволяет узлу сформировать адрес по объявлениям
  маршрутизатора; DHCPv6 может дополнять или заменять эту схему.
...
> **Зачем это Go-разработчику.** Для нового кода удобно использовать
> `net/netip`: типы `netip.Addr` и `netip.Prefix` сравнимы, компактны и явно
> различают адрес и подсеть. Стандартный `net.Dialer` умеет пробовать IPv6 и IPv4,
> чтобы неисправный путь IPv6 не создавал долгую задержку.
...
**Шлюз по умолчанию (default gateway)** — маршрут `0.0.0.0/0`, через который идут все пакеты, не попавшие в более специвозможностейные маршруты. Обычно это IP-адрес роутера в локальной сети.
...
NAT не является межсетевым экраном и не задаёт политику безопасности. То, что
некоторые реализации не имеют отображения для нового входящего соединения, —
побочный эффект состояния трансляции, а не защита, на которую следует полагаться.
...
* **Надёжный поток или ошибка**: TCP подтверждает и повторно передаёт данные, но
  не обещает, что удалённое приложение их обработает. При неустранимом сбое
...
После активного закрытия одна сторона обычно некоторое время находится в
состоянии `TIME_WAIT`, чтобы старые сегменты не попали в новое соединение с той
же четвёркой адресов и портов. Реализации TCP обычно встроены в ядра ОС, хотя
существуют и варианты в пользовательском пространстве.
...
| Надёжность          | Упорядоченный поток либо ошибка        | Доставка не гарантирована       |
| Порядок             | Сохраняется                           | Не сохраняется                  |
| Накладные расходы   | Больше состояния и служебной работы   | Меньший заголовок; скорость зависит от протокола приложения |
...
> **Зачем это Go-разработчику.** Выбор TCP vs UDP — архитектурное решение. TCP для всего, где целостность важнее скорости. UDP для реального времени и loss-tolerant сценариев. В Go оба протокола используют схожий API через `net.Conn`, что упрощает смену транспорта.

### Практические правила для `net.Conn`

TCP передаёт поток байтов и не сохраняет границы вызовов `Write`. Один `Read`
может вернуть лишь часть сообщения или сразу данные нескольких сообщений.
Протокол приложения поэтому должен задавать границы: длиной перед телом,
разделителем либо форматом с собственным кадрированием. Для чтения известного
числа байтов используйте `io.ReadFull`; всегда обрабатывайте одновременно `n > 0`
и ненулевую ошибку.

Операции сети без конечного срока способны ждать неограниченно. Устанавливайте
`SetDeadline`, `SetReadDeadline` или `SetWriteDeadline` и обновляйте срок по мере
работы долгоживущего соединения. Срок относится к соединению, а не только к
одному следующему вызову.

Размер IP-пакета ограничивает MTU пути. TCP обычно выбирает MSS так, чтобы
сегменты помещались в MTU; прикладному коду не следует считать, что один `Write`
соответствует одному пакету. Для UDP слишком большая датаграмма может быть
фрагментирована или потеряна, поэтому предел размера должен быть частью
...
1. Клиент → **DNS-резолвер** (обычно ваш роутер или публичный DNS вроде `8.8.8.8`): «Какой IP у `www.example.com`?»
...
6. Рекурсивный сервер → **авторитетный сервер example.com**: «Какой IP у `www.example.com`?»
...
**WebSocket** — протокол полнодуплексной связи поверх TCP, разработанный для реального времени взаимодействия между браузером и сервером. В отличие от HTTP (запрос-ответ), WebSocket позволяет серверу отправлять данные клиенту в любой момент без предварительного запроса.

### Зачем WebSocket, если есть HTTP

HTTP — полудуплексный протокол: клиент запрашивает, сервер отвечает. Для реального времени сценариев (чат, биржевые котировки, игры) приходилось использовать обходные пути:
...
> **Зачем это Go-разработчику.** WebSocket — стандарт для реального времени возможностей: чаты, уведомления, оперативные обновления, коллаборативное редактирование. В Go: `github.com/gorilla/websocket` — основная библиотека. Типичный паттерн: HTTP-сервер для обычных запросов + WebSocket-эндпоинт для реального времени. В микросервисной архитектуре WebSocket-соединения требуют особого подхода к масштабированию (sticky sessions, message broker).

***

## 11. Транспорт и имена в рабочей системе

### Управление потоком и перегрузкой TCP

TCP решает две разные задачи:

- **управление потоком** не даёт отправителю переполнить буфер получателя;
- **управление перегрузкой** ограничивает нагрузку на саму сеть.

Фактический объём данных в пути ограничен меньшим из окна получателя и
окна перегрузки. Потерянный или неподтверждённый сегмент передаётся повторно
по истечении рассчитанного срока или после событий, указывающих на потерю.
Приложение не видит отдельные повторы: оно читает упорядоченный поток байтов.

**MSS** ограничивает полезную нагрузку TCP-сегмента, а **MTU** — весь пакет канального
уровня. Слишком большие IP-пакеты могут потребовать фрагментации или не пройти
по пути. Поэтому протокол приложения должен задавать разумный предел сообщения,
даже если транспорт технически примет больше.

### Завершение TCP-соединения

Каждое направление TCP закрывается отдельно через FIN и подтверждение ACK. Состояние
`TIME_WAIT` защищает новое соединение с той же четвёркой адресов от запоздалых
сегментов. RST завершает соединение аварийно и может привести к потере данных,
которые ещё не были подтверждены.

### IPv6: обнаружение соседей и выбор адреса

IPv6 использует **NDP** поверх ICMPv6 вместо ARP. Маршрутизатор объявляет префиксы;
узел может сформировать адрес через **SLAAC** или получить параметры через DHCPv6.

Если DNS вернул A и AAAA, клиент не должен слепо ждать отказа одной семьи адресов.
Алгоритм **Happy Eyeballs** перемежает попытки IPv6 и IPv4 с небольшой задержкой,
чтобы неисправный путь не делал подключение заметно медленнее.

### DNS: кеш, защита и зашифрованный транспорт

Положительный ответ кешируется на срок TTL. Отсутствие имени или записи тоже может
кешироваться — это **отрицательный кеш**. После создания ранее отсутствовавшей записи
потребители могут ещё некоторое время видеть старый отрицательный ответ.

- **DNSSEC** позволяет проверить подпись данных, но не шифрует запрос;
- **DoT** передаёт DNS в TLS-соединении;
- **DoH** передаёт DNS по HTTP поверх TLS.

Шифрование канала не делает ответ истинным само по себе: нужно доверять резолверу
или проверять DNSSEC.

### TLS, ALPN и QUIC

TLS защищает конфиденциальность и целостность потока и подтверждает подлинность
узла по цепочке сертификатов. **ALPN** во время TLS-рукопожатия выбирает прикладной
протокол, например `h2` или `http/1.1`.

QUIC работает поверх UDP, встраивает TLS 1.3 и имеет независимые потоки. Потеря пакета
в одном потоке не обязана останавливать доставку других потоков, как это происходит
при мультиплексировании над одним TCP-потоком. HTTP/3 использует QUIC как транспорт.

### Адреса и разрешение имён в Go

`net/netip` даёт сравнимые значения `Addr`, `AddrPort` и `Prefix` без двусмысленности
изменяемого `net.IP`:
...
if !prefix.Contains(addr) {
    return errors.New("адрес вне разрешённой сети")
}
```

`net.Resolver` позволяет передать контекст, но кеширование и политика повторов остаются
задачей системного резолвера или приложения:
...
if err != nil {
    return fmt.Errorf("разрешить имя: %w", err)
}
```

Для TCP нужно также задать границы сообщений. Например, четырёхбайтный префикс длины
позволяет применить `io.ReadFull`, проверить предел до выделения памяти и затем прочитать
ровно тело сообщения.
...
**Основные источники:** [TCP — RFC 9293](https://www.rfc-editor.org/rfc/rfc9293) и [IPv6 — RFC 8200](https://www.rfc-editor.org/rfc/rfc8200).

## Краткий итог

- TCP передаёт поток байтов, поэтому границы сообщений задаёт прикладной протокол.
- Сроки подключения, чтения и записи ограничивают разные этапы сетевого обмена.
- DNS, маршрутизация и транспорт могут завершаться независимо и требуют раздельной диагностики.

Дальше: разберите [семантику HTTP и работу `net/http`](../../architecture/HTTP/http.md).
